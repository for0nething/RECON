...
#     colValue = np.floor(colValue)
    colValue = np.around(arrLike[col],0)
    
    colValue = np.floor(colValue/2)
    return colValue
#     purchase = parseDatetime(arrLike[col1])
#     approve = parseDatetime(arrLike[col2])
#     delta = approve - purchase
...
def LoadIMDBC(Large=0,dataName="", saveCSV=False):
...
#     trainSize = int(0.5 * z.shape[0])
#     valSize = trainSize + int(0.25 * z.shape[0])
...
dataNameList = ["IMDBC5", "IMDBLargeC5"]
parameterList = [0, 1]

for dataName, param in zip(dataNameList, parameterList):
    LoadIMDBC(param, dataName, saveCSV=True) 
